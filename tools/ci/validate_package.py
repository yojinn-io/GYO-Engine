#!/usr/bin/env python3
"""Check installed native linkage in isolation; application content checks are app-owned."""
...
from package_contract import load_manifest, required_files


def validate_linkage(stage: Path, app: str, logs: Path, dumpbin: str | None = None) -> None:
    stage, logs = stage.resolve(strict=True), logs.resolve()
    logs.mkdir(parents=True, exist_ok=True)
    manifest = load_manifest(stage, app)
    ...
    with TemporaryDirectory(prefix="gyo-package-linkage-") as temporary:
        ...
        executable = str(package / manifest["executable"])
        if platform.system() == "Windows":
            ...
            if missing:
                raise RuntimeError("Package is missing app-local MSVC runtime DLLs:\n" + "\n".join(sorted(missing)))
...
def main():
    ...
    validate_linkage(args.stage, args.app, args.logs, args.dumpbin)
