...
constexpr std::string_view kUiFont = "object_fps_pvp.font.ui";
...
constexpr std::array<UiActionContractEntry, 8> kUiActionContract{{
    {"object_fps_pvp.start_game", Engine::Ui::UiActionPayloadType::None},
    {"object_fps_pvp.open_controls", Engine::Ui::UiActionPayloadType::None},
    {"object_fps_pvp.close_controls", Engine::Ui::UiActionPayloadType::None},
    {"object_fps_pvp.resume", Engine::Ui::UiActionPayloadType::None},
    {"object_fps_pvp.return_main_menu", Engine::Ui::UiActionPayloadType::None},
    {"object_fps_pvp.quit", Engine::Ui::UiActionPayloadType::None},
    {"object_fps_pvp.set_gamma", Engine::Ui::UiActionPayloadType::Number},
    {"object_fps_pvp.set_exposure", Engine::Ui::UiActionPayloadType::Number},
}};
...
bool ObjectFpsUi::Update(
    ...
    for (const Engine::Ui::UiActionEvent& event : updated.value()) {
        if (event.action == "object_fps_pvp.start_game") {
            commands.emplace_back(StartCampaignCommand{});
        } else if (event.action == "object_fps_pvp.open_controls") {
            commands.emplace_back(OpenControlsCommand{});
        } else if (event.action == "object_fps_pvp.close_controls") {
            commands.emplace_back(CloseControlsCommand{});
        } else if (event.action == "object_fps_pvp.resume") {
            commands.emplace_back(ResumeCommand{});
        } else if (event.action == "object_fps_pvp.return_main_menu") {
            commands.emplace_back(ReturnToMainMenuCommand{});
        } else if (event.action == "object_fps_pvp.quit") {
            commands.emplace_back(RequestQuitCommand{});
        } else if (event.action == "object_fps_pvp.set_gamma") {
        ...
        } else if (event.action == "object_fps_pvp.set_exposure") {
            const double* value = std::get_if<double>(&event.payload);
            if (value == nullptr) {
