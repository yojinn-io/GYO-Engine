...
class InstalledPackageTests(unittest.TestCase):
    def setUp(self):
        ...
        self.manifest_path = self.stage / "share/gyo/apps" / self.app / "manifest.json"
        self.manifest_path.parent.mkdir(parents=True)
        self.manifest = {"schema_version": 1, "app": self.app,
                         "executable": self.relative_executable}
    ...
    def test_discovers_unfamiliar_app_and_actual_executable_name(self):
        ...
        self.assertEqual(result["executable"], self.relative_executable)

    def test_installed_clis_load_sibling_helper_with_isolated_python(self):
        scripts = Path(__file__).resolve().parents[1]
        for name in ("package_info.py", "manual_gpu_smoke.py", "validate_content.py"):
            shutil.copy2(scripts / name, self.stage / name)
        for script in ("manual_gpu_smoke.py", "validate_content.py"):
            with self.subTest(script=script):
                result = subprocess.run([sys.executable, "-I", str(self.stage / script), "--help"],
    ...
    def test_missing_and_ambiguous_manifest_fail(self):
        ...
        self.write_manifest()
        other = self.stage / "share/gyo/apps/another_game/manifest.json"
        other.parent.mkdir()
    ...
    def test_rejects_wrong_schema_or_directory_identity(self):
        for replacement in ({"schema_version": True}, {"schema_version": 2},
                            {"app": "other_app"}, {"app": "../escape"}, {"app": None}):
    ...
    def test_rejects_unsafe_or_non_normalized_executable_paths(self):
        for executable in ("", ".", "../outside", "bin/../outside", "/outside", "C:/outside",
            ...
            with self.subTest(executable=executable):
                self.manifest["executable"] = executable
                self.write_manifest()
    ...
    def test_gpu_cli_uses_manifest_executable_as_one_argument(self):
        ...
        with patch.object(gpu, "run_suite", return_value=results) as run_suite:
            result = gpu.main(["--package", str(self.stage), "--driver", "vulkan",
    ...
    def test_gpu_cli_rejects_ambiguous_package_before_diagnostics(self):
        ...
        with patch.object(gpu, "run_suite") as run_suite, redirect_stderr(io.StringIO()):
            with self.assertRaises(SystemExit) as failure:
                gpu.main(["--package", str(self.stage), "--driver", "vulkan",
    ...
    def test_content_checks_follow_manifest_and_preserve_original_install(self):
        ...
        with patch.object(content.tempfile, "TemporaryDirectory", return_value=nullcontext(str(isolated))), \
                patch.object(content.subprocess, "run", side_effect=run), redirect_stdout(io.StringIO()):
            content.validate(self.stage, self.root / "content logs")
    ...
    def test_content_rejects_false_success_and_restores_hidden_content(self):
        ...
        with patch.object(content.tempfile, "TemporaryDirectory", return_value=nullcontext(str(isolated))), \
            ...
            with self.assertRaisesRegex(RuntimeError, "missing-common"):
                content.validate(self.stage, self.root / "content logs")
        self.assertTrue((isolated / "package/bin/assets/common/asset_catalog.json").is_file())
        self.assertTrue((self.stage / "bin/assets/common/asset_catalog.json").is_file())
