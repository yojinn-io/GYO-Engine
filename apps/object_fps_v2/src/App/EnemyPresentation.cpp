...
bool EnemyPresentation::Initialize(Engine::Render::IRenderDevice& device,
    ...
    try {
        ...
        for (const auto& enemy : catalog.GetDefinitions()) {
            ...
            auto character =
                LoadCharacterPresentationDefinition(assets, enemy.rig->characterAssetId, error);
            if (!character)
            ...
            auto made = Impl::Resource::Create(device, enemy.rig->model, materials);
            if (!made)
                throw std::runtime_error(made.error());
            next->definitions.emplace(enemy.id,
                                      Impl::Definition{enemy.rig, std::move(made.value())});
        }
        impl_ = std::move(next);
        return true;
