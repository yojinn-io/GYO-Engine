...
std::vector<Engine::Collision::Aabb> BuildWorldCollisionBoxes(const GridMap &map,
    ...
    // GridMap treats cells outside its extent as solid, including open-edge maps.
    // Keep that product contract explicit in the same geometry sent to debug draw.
