...
def main():
    ...
    matrix = {"include": [dict(product=product, kind="app", platform=platform, **TOOLCHAINS[platform]) for product, platform in pairs]}
    baseline = {"include": [dict(product="toolchain", kind="toolchain", platform=platform, **TOOLCHAINS[platform]) for platform in PLATFORMS]}
    if args.output:
