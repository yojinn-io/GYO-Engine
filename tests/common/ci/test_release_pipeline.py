...
def archive_name(platform, product=APP):
    return _archive_name(product, platform)


def validate_archive(name, data, platform, commit, product=APP):
    return _validate_archive(name, data, product, platform, commit)
...
def package(platform="windows-x64", commit=COMMIT, *, product=APP, extra=(), extra_files=(), metadata_changes=None,
            timestamp=0, omit=(), empty=(), executable_mode=0o755):
    contract = manifest(platform, product)
    metadata = {"product": product, "kind": contract["kind"], "source_revision": commit, "platform": platform,
                "ci_smoke": evidence(contract, commit)}
    inventory = {name:{"sha256":hashlib.sha256(b"" if name in empty else data).hexdigest()}
                 for name,data in package_contents(platform, product).items() if name not in omit}
    for path,data in extra_files:
        inventory[path] = {"sha256":hashlib.sha256(data).hexdigest()}
    for member in extra:
        if member.issym():
            root_prefix = "gyo-" + product + "/"
            relative = member.name.removeprefix(root_prefix)
            inventory[relative] = {"link":member.linkname}
    metadata["ci_smoke"]["package_sha256"] = inventory_digest(inventory)
    ...
    with tarfile.open(fileobj=output, mode="w:gz") as archive:
        ...
        for path, data in package_contents(platform, product).items():
            if path in omit:
                continue
        ...
        for path, data in extra_files:
            ...
            member.size = len(data)
            archive.addfile(member, io.BytesIO(data))
    name = archive_name(platform, product)
    data = output.getvalue()
    return Package(product, platform, name, data, checksum_document(name, data))
...
class PackageTests(unittest.TestCase):
    ...
    def test_dynamic_matrix_supports_multiple_apps_and_asymmetric_platforms(self):
        inputs = [package("linux-x64"), package("windows-x64", product="second_app")]
        self.write_packages(inputs)
        expected = [(item.product, item.platform) for item in inputs]
    ...
    def test_archive_rejects_another_app_installation_even_with_valid_own_evidence(self):
        other = json.dumps(manifest(product="other_app")).encode()
        item = package(extra_files=[(manifest_path("other_app"), other)])
        with self.assertRaisesRegex(ReleaseError, "another product installation: other_app"):
            validate_archive(item.name, item.data, item.platform, COMMIT)
...
class DraftTests(unittest.TestCase):
    ...
    def test_unexpected_managed_assets_block_all_remote_mutation(self):
        extra = package(product="other_app", commit=OTHER_COMMIT)
